 ...
 ## Ultimo coisa é a implementação do ensaio por teste
# Ultima função excluirlinha_grafic codada não está funcional 
import sys 
#import Programa 
from PyQt4 import QtCore, QtGui
import time
# Mudar cor label: nom_plan_label.setStyleSheet('color: yellow')
#import numpy as np
...
# .setRange(0,1000) # coloca maximo para 1000 do  QSpinBox
...
class Ui_MainWindow(object):
    ...
    def __init__(self):
        ...
        #self.result.setInformativeText("Teste em Lotes")
        self.result.addButton(QtGui.QMessageBox.Yes)
        self.result.addButton(QtGui.QMessageBox.No)
        
        

        '''QtGui.QMessageBox.question(None,"Teste em Lotes",\
                                   "Você deseja fazer mais um teste nesse lote?",\
    ...
    def Parando(self):
        ...
        '''result=QtGui.QMessageBox.question(None,"Teste em Lotes",\
                                   "Você deseja fazer mais um teste nesse lote?",\
    ...
    def excluirlinha_grafic(self):
        ...
        #print(self.Index)
        self.line1.set_data([],[])
        #self.ax1f1.legend().remove()
        self.combo2.removeItem(self.Index)
        #print("aquiooooo")
        ##print(int(self.returnposteste(self.Index)))
    ...
    def update1(self,lista):
                        ...
                        #self.emit(QtCore.SIGNAL("dosomething(QString)"), lista)
                        #self.fig.canvas.ion()
                        
                        self.Deslocamento_grafic.setValue(lista[0])
                        self.Forca_grafic.setValue(lista[1])
                        #self.ax1f1.set_ylim(0, 150)
                        #self.ax1f1.set_xlim(0, 100)
                        self.ax1f1.set_ylim(0, lista[2])
                        self.ax1f1.set_xlim(0, lista[3])
                        #self.ax1f1.relim()
                        self.line1.set_data(lista[4],lista[5])
                        """self.line1.set_data([],[])
                        "self.line1, = self.ax1f1.plot(lista[4],lista[5])"""
...
"""class ThreadGrafic(QtCore.QThread):
