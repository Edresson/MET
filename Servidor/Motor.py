...
def calcular(valor,vfrequencia):
    global frequencia
    frequencia = float(vfrequencia)
    thr= thread()
    valor = float(valor)
    velocidadei = valor;
...
def get_pontos(frequencia_teste):
    ...
    while (time() - inicio < 1000):#manter a maquina 1 minuto ligada
        pass
...
def Subir_descer(vel,freq,controle,deslocamento):
    if float(deslocamento) != 0:
